...
def check_file_size_hook(d):
    ...
    file_size_threshold = 6 * 1024 * 1024  # Set your desired threshold in bytes (e.g., 100 MB)

    if d.get('downloaded_bytes') and d['downloaded_bytes'] > file_size_threshold:
        raise Exception(f"File size exceeded the threshold of {file_size_threshold} bytes. Skipping to the next song.")
