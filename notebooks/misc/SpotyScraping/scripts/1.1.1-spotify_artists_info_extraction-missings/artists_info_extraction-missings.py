...
for i, row in tqdm.tqdm(artists_df_missings.copy().iterrows(), total=df_size):
    ...
    if i % 500 == 0 and i != 0 and rows:
        ...
        with open(
            f"../1.1-spotify_artists_info_extraction/extracted_artists_info/missings/missings_{timestamp_}.pickle",
            "wb",
        ) as f:
...
new_missings = artists_df_missings[
    ~artists_df_missings["artist_id"].isin(set(ids_found))
]
