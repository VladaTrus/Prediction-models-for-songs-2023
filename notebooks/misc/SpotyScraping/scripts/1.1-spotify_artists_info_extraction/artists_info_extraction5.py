...
artists_df_global = pd.read_csv(
    "../../data/generated/1.1-spotify_artist_ids/artists_ids.csv"
)
...
with open("../../tokens/spotify_tokens.json", "r") as f:
    ...
    keys = [str(i + 1) for i in range(5 * (PT - 2), 5 * (PT - 2) + 5)]
    for key in keys:
...
for i, row in tqdm.tqdm(artists_df_current.copy().iterrows(), total=df_size):
    ...
    if "error" in resp and resp["error"]["message"] in {
        ...
        row[["followers", "genres", "image_url", "popularity"]] = [
            followers,
            genres,
            image_url,
            popularity,
    ...
    if i % 500 == 0 and i != 0:
        ...
        with open(
            f"extracted_artists_info/pt_{PT}/artists_info_{i-500}_{i}.pickle", "wb"
        ) as f:
...
if rows:
    sub_df = pd.DataFrame(rows)
    with open(
        f"extracted_artists_info/pt_{PT}/artists_info_{df_size - (df_size % 500)}_{df_size}.pickle",
        "wb",
    ) as f:
        pickle.dump(sub_df, f)
