...
artists_df_global = pd.read_csv("../../data/generated/1.2-spotify_artists_info/artists_info.csv")
...
for i, artist_id in tqdm.tqdm(enumerate(artist_ids), total=df_size):
    ...
    if "error" in resp and resp['error']['message'] in {"Invalid access token", "The access token expired"}:
        ...
        for item_ in resp['items']:
            ...
            artists = "|".join([i['id'] for i in item_['artists'] if i['type'] == "artist"])
            album_id = item_['id']
            album_name = item_['name']
            release_date = item_['release_date']
            n_tracks = item_['total_tracks']
            
            rows.append([artist_id, album_id, album_name, album_type, artists, release_date, n_tracks])
    
    if i % SAVE_CHUNK_SIZE == 0 and i != 0:
        sub_df = pd.DataFrame(rows, columns=['artist_id', 'album_id', 'album_name', 'album_type', 'artists', 'release_date', 'n_tracks'])
        with open(f"extracted_albums_info/pt_{PT}/albums_info{i-SAVE_CHUNK_SIZE}_{i}.pickle", "wb") as f:
